...
要以式读文件的模打开一个文件对象，使用Python内置的`open()`函数，传入文件名和标示符：
...
`Pickle`的问题和所有其他编程语言特有的序列化问题一样，就是它只能用于Python，并且可能不同版本的Python彼此都不兼容，因此，只能用`Pickle`保存那些不重要的数据，不能成功地反序列化也没关系。
...
别急，我们仔细看看`dumps()`方法的参数列表，可以发现，除了第一个必须的obj参数外，`dumps()方`法还提供了一大堆的可选参数：https://docs.python.org/3/library/json.html#json.dumps
